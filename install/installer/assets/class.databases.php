<?php if(!defined('INST_BASEDIR')) die('Direct access is not allowed!');
    ...
    *  ================================================================= */
    ...
    class Inst_Databases
    {
        ...
        /** 
		 *  class constructor Connects to database and executes queries 
		 */
        ...
        function Connect($host='', $user='', $pass='', $port='')
        {
			...
			if(is_array($host))
			{
				...
				$this->connection = mysqli_connect($hostport, $host['username'], $host['password']);            
			}
			else 
			{
				if(strlen($port) > 0)
        ...
        function RunQuery($query)
        {
			if(is_array($query))
			{
				...
				foreach($query as $q)
				{
					...
					foreach($q as $query)
					{
						$result = mysqli_query( $this->connection, $query);// BYOOS_tag modif mysqli_query for paramters $this->connection
					}
					...
					if($result){
						...
						$results[] = array('success'=>false, 'data'=>$result, 'error'=>$this->GetDatabaseError());
					}
				}
				return $results;
			}
			...
			{
				return mysqli_query( $this->connection, $query);// BYOOS_tag modif mysqli_query for paramters $this->connection
			}
        }
        ...
        {
            $this->dbselected = mysqli_select_db($this->connection, $database);  
            ...
            if($this->IsConnected())
            {
				...
				if($result)
				{
					while ($row = mysqli_fetch_object($result)) 
					{
						...
						$dblist[] = array(
							'name'  => $row->Database, 
							'tbcount' => is_countable( $tbresult ) ? mysqli_num_rows($tbresult) : false
					...
					if($includeSelectedDatabase && $this->IsDatabaseSelected())
					{
						...
						if($notInList)
						{
							...
							$dblist[] = array(
								...
								'tbcount' => mysqli_num_rows($tbresult)
								);
						}
					}
